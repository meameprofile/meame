  ...
  const play = useCallback(() => {
    ...
    try {
      if (typeof window.Audio === "undefined") {
        logger.warn("[Sound] El API de Audio no está disponible en este entorno.", { traceId });
        return;
      }
      const audio = new Audio(soundPath);
      audio.volume = volume;
      audio.play().catch(error => {
        logger.error("[Sound] Fallo al reproducir el sonido.", { error, soundPath, traceId });
      });
      logger.trace(`[Sound] Reproduciendo sonido: ${soundPath}`, { traceId });
    } catch (error) {
      logger.error("[Sound] Error inesperado al intentar reproducir sonido.", { error, traceId });
    } finally {

...
export function useAssetExplorerLogic() {
  const traceId = useMemo(
    () => logger.startTrace("useAssetExplorerLogic_v4.0"),
    []
  );
  useEffect(() => {
  ...
  const fetchAssets = useCallback((input: GetBaviAssetsInput) => {
    startTransition(async () => {
      ...
      logger.info("[AssetExplorer] Iniciando fetch de activos...", {
        input,
      ...
      if (result.success) {
        ...
        logger.success(
          `[AssetExplorer] Fetch exitoso. ${result.data.assets.length} activos cargados.`,
          { traceId: fetchTraceId }
        );
      } else {
        ...
        setAssets([]);
        setTotalAssets(0);
        logger.error("[AssetExplorer] Fetch fallido.", {
          error: result.error,
          traceId: fetchTraceId,
        });
      }

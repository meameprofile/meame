...
export function useNos3Tracker(enabled: boolean): void {
  ...
  const flushEvents = useCallback(async (isUnloading = false) => {
    ...
    logger.traceEvent(flushTraceId, `[nos3] Vaciando ${eventsToSend.length} eventos para ${sessionId}.`);
  ...
  useEffect(() => {
    logger.info(`[useNos3Tracker] Hook montado. Estado: ${enabled ? 'HABILITADO' : 'DESHABILITADO'}.`, { traceId });

    if (!enabled || isRecording.current) {
      if (!enabled) logger.traceEvent(traceId, "Grabación deshabilitada, no se iniciará.");
      return;
    }

    logger.success("[nos3] Condiciones cumplidas. Iniciando grabación.", { traceId });
    isRecording.current = true;

    const intervalId = setInterval(() => flushEvents(false), BATCH_INTERVAL_MS);
    const stopRecording = rrweb.record({
      emit(event) { eventsBuffer.current.push(event as eventWithTime); },
      maskAllInputs: true, blockClass: "nos3-block", maskTextClass: "nos3-mask",
    });

...
export async function getPromptsAction(
  ...
  try {
    ...
    if (!validatedInput.success) return { success: false, error: "Parámetros inválidos." };

    const { page, limit, query, tags, workspaceId } = validatedInput.data;

    const { data: memberCheck, error: memberError } = await supabase.rpc(
      "is_workspace_member", { workspace_id_to_check: workspaceId }
    );
    if (memberError || !memberCheck) throw new Error("Acceso denegado al workspace.");

    let queryBuilder = supabase.from("razprompts_entries")
      .select("*, count()", { count: "exact" })
      .eq("workspace_id", workspaceId);

    if (query) queryBuilder = queryBuilder.or(`title.ilike.%${query}%,keywords.cs.{${query.split(" ").join(",")}}`);
    if (tags) Object.entries(tags).forEach(([k, v]) => v && queryBuilder.eq(`tags->>${k}`, v));
    ...
    const validatedPrompts: RaZPromptsEntry[] = (data as RazPromptsEntryRow[] || []).map(row => {
      try {
        return mapSupabaseToCamelCase(row, traceId);
      } catch { return null; }
    }).filter((p): p is RaZPromptsEntry => p !== null);

    const assetIdsToFetch = Array.from(new Set(
      validatedPrompts.flatMap(p => p.baviAssetIds?.[0] ? [p.baviAssetIds[0]] : [])
    ));
    ...
    if (assetIdsToFetch.length > 0) {
      const { data: variantsData, error: variantsError } = await supabase
        .from("bavi_variants").select("asset_id, public_id")
        .in("asset_id", assetIdsToFetch).eq("state", "orig");

      if (variantsError) logger.warn("[Action] Fallo parcial al obtener variantes de BAVI.", { error: variantsError.message, traceId });
      else variantsData?.forEach(v => assetIdToPublicIdMap.set(v.asset_id, v.public_id));
    }
    ...
    const enrichedPrompts = validatedPrompts.map(prompt => {
      ...
      if (primaryAssetId && assetIdToPublicIdMap.has(primaryAssetId)) {
        ...
        return { ...prompt, primaryImageUrl: `https://res.cloudinary.com/${process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME}/image/upload/f_auto,q_auto,w_400/${publicId}` };
      }
      return prompt;
    });

    return { success: true, data: { prompts: enrichedPrompts, total: count ?? 0 } };

  } catch (error) {
    const msg = error instanceof Error ? error.message : "Error desconocido.";
    logger.error("[Action] Fallo crítico al obtener prompts.", { error: msg, traceId });
    return { success: false, error: `No se pudieron cargar los prompts: ${msg}` };
  } finally {
    logger.endGroup();

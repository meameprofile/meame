...
export async function getHeimdallInsightAction(input: { eventId: string }): Promise<ActionResult<string>> {
    const traceId = logger.startTrace("getHeimdallInsightAction_v1.2");
    // --- [INICIO DE CORRECCIÓN DE CONTRATO v1.2.0] ---
    const groupId = logger.startGroup(`[AI Action] Analizando evento: ${input.eventId}`);
    ...
    try {
        ...
        if (!validation.success) return { success: false, error: "ID de evento inválido." };

        const { data: eventData, error } = await supabase.from("heimdall_events").select("*").eq("event_id", validation.data.eventId).single();
        if (error || !eventData) throw new Error("Evento no encontrado.");

        const promptPath = path.join(process.cwd(), "prompts", "analyze-heimdall-event.md");
        const masterPrompt = await fs.readFile(promptPath, "utf-8");

        const finalPrompt = `${masterPrompt}\n\n--- HEIMDALL EVENT DATA ---\n\n${JSON.stringify(eventData, null, 2)}`;

        const result = await gemini.generateText({ prompt: finalPrompt, modelId: "gemini-1.5-flash" });
        if (!result.success) return result;

        // Limpieza robusta de la respuesta JSON de la IA
        const cleanJsonString = result.data.replace(/```json\n?|\n?```/g, "").trim();
        ...
        logger.error("[AI Action] Fallo en el análisis de Heimdall.", { error: msg, traceId });

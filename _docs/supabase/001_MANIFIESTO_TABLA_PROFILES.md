...
---
// RUTA: _docs/supabase/001_MANIFIESTO_TABLA_PROFILES.md
/**
 * @file 001_MANIFIESTO_TABLA_PROFILES.md
 * @description Manifiesto Canónico y SSoT para la tabla 'public.profiles' y la arquitectura de identidad híbrida.
 * @version 5.0.0 (Consolidado)
 * @author RaZ Podestá - MetaShark Tech
 */
...
Esta tabla es la extensión soberana de la entidad `auth.users`. Almacena metadatos de aplicación que definen la identidad, la actividad y las preferencias de un usuario en nuestro ecosistema. Su poder reside en su simbiosis con el sistema de **"Sesión Ascendente"**, que vincula el viaje anónimo de un visitante con su identidad autenticada.
...
Política de ACTUALIZACIÓN (UPDATE): Un usuario solo puede actualizar su propio perfil.
WITH CHECK (auth.uid() = id)
Política de INSERCIÓN (INSERT): Un usuario solo puede crear su propio perfil.
WITH CHECK (auth.uid() = id)
4. Triggers y Funciones Asociadas
handle_new_user: Un trigger en auth.users que se dispara AFTER INSERT para crear automáticamente la fila correspondiente en public.profiles, garantizando la relación 1:1.
on_profile_update: Un trigger que se dispara BEFORE UPDATE en public.profiles para actualizar automáticamente el campo updated_at.

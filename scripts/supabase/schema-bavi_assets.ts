...
async function diagnoseBaviAssetsSchema(): Promise<ScriptActionResult<string>> {
  ...
  const report: Report = {
    ...
    instructionsForAI: [
      `Este es un informe de diagnóstico estructural para la tabla de base de datos '${TARGET_TABLE}', el corazón de la Biblioteca de Activos Visuales (BAVI).`,
      "Analiza la sección 'schemaDetails' para entender la arquitectura completa de la tabla.",
      "'columns': Detalla cada columna y su tipo de dato. Verifica la existencia de 'asset_id', 'workspace_id', 'user_id', 'prompt_id', 'tags', y 'metadata'.",
      "'constraints': Valida que 'asset_id' sea la PRIMARY KEY y que existan FOREIGN KEYs hacia 'workspaces' y 'users'.",
      "'indexes': Revisa si existen índices que optimicen las búsquedas por 'workspace_id' y 'user_id'.",
      "'rls_policies': Confirma que las políticas de Seguridad a Nivel de Fila (RLS) están activas para restringir el acceso por workspace.",

// pnpm tsx scripts/run-with-env.ts scripts/supabase/schema-workspaces.ts
/**
...
async function diagnoseWorkspacesSchema(): Promise<ScriptActionResult<string>> {
  ...
  const report: Report = {
    ...
    instructionsForAI: [
      ...
      "Analiza 'columns' para verificar la existencia de 'id' (UUID, PK) y 'owner_id' (UUID, FK a auth.users).",
      "'constraints': Valida que 'id' sea la PRIMARY KEY y que 'owner_id' tenga una FOREIGN KEY.",
      "'rls_policies': Es CRÍTICO verificar que las políticas de seguridad RLS restrinjan el acceso (SELECT, UPDATE, DELETE) a los miembros del workspace, utilizando la función 'is_workspace_member'.",

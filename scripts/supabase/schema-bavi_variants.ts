...
async function diagnoseBaviVariantsSchema(): Promise<ScriptActionResult<string>> {
  const TARGET_TABLE = "bavi_variants";
  const traceId = scriptLogger.startTrace(`diagnoseSchema:${TARGET_TABLE}`);
  scriptLogger.startGroup(`🔬 Auditando Esquema de la Tabla: '${TARGET_TABLE}'...`);
  ...
  const report: Report = {
    ...
    auditStatus: "FAILED",
    schemaDetails: { columns: [], constraints: [], indexes: [], rls_policies: [] },
    summary: "",
  };

  try {
    const supabase = createScriptClient();
    scriptLogger.info(`Invocando RPC 'get_system_diagnostics' para filtrar por '${TARGET_TABLE}'...`);
    ...
    report.schemaDetails.columns = diagnosticsData.schema_columns.filter((c) => c.table === TARGET_TABLE);
    report.schemaDetails.constraints = diagnosticsData.table_constraints.filter((c) => c.table === TARGET_TABLE);
    report.schemaDetails.indexes = diagnosticsData.indexes.filter((i) => i.table === TARGET_TABLE);
    report.schemaDetails.rls_policies = diagnosticsData.rls_policies.filter((p) => p.table === TARGET_TABLE);

    if (report.schemaDetails.columns.length === 0) {
      throw new Error(`La tabla '${TARGET_TABLE}' no fue encontrada en el esquema de la base de datos.`);
    }
